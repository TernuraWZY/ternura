...
- History 按钮：打开可滚动历史弹窗，用卡片隔离每个历史 run，并以状态标签、输入摘要、回复预览和 `run_id` 展示内容，支持一键恢复或定位
- 主对话区域：展示用户消息、模型回复和 `<think>` 内容
- 底部输入区：固定在控制台底部，用更紧凑的输入框发送新的任务或问题
- Reset chat：重置当前内存会话和本地历史会话
...
- 可恢复会话：刷新页面后会自动恢复历史消息；也可以通过顶部 `History` 按钮打开弹窗，手动恢复或定位历史 run
...
前端默认使用 `/api/chat/stream` 接收 `text/event-stream`。旧的 `/api/chat` JSON 接口仍然保留，便于调试或接入非流式客户端。`/api/history` 会返回已持久化的运行历史，供页面刷新后恢复展示。
...
- `runs`：每一轮请求的 `run_id`、状态、用户输入、最终回复、原始回复、trace、错误信息和耗时
- `messages`：用于服务重启后恢复模型上下文的用户/assistant 对话历史

`.ternura/` 已经被 `.gitignore` 忽略，不会被提交到仓库。点击 `Reset chat` 会同时清空当前内存会话和 `.ternura/session.json`。
...
Web Console 会把每一轮运行写入本地会话文件。页面刷新时，前端会读取 `/api/history` 并重新渲染历史对话；服务重启时，后端也会从持久化的 `messages` 恢复模型上下文，让后续提问能接上之前的会话。
...
- [ ] 计划与步骤状态：让模型或后端维护任务 plan，把步骤展示为 pending/running/done/failed，方便用户理解进度
...
```bash
gofmt -w agent.go main/main.go main/session_store.go shared/config.go tool/*.go prompt.go
```
