...
- 内置状态归因 Guard：用模型生成的 claim/evidence 合约校验最终回复，防止模型把未执行的操作、未查询的外部事实或旧上下文说成当前事实
...
这些默认值用于防止单轮交互在工具调用、网页抓取和 grounding 校验之间无限拉长。`web_fetch` 现在会对搜索结果页、验证码/反爬页面和非 2xx HTTP 响应快速标记为不可用证据；达到抓取上限时会提示“没有 fetch 到更多有效网页信息”，模型应基于已有证据停止探索并说明未验证部分。
...
Harness 还会执行通用工具证据校验：最终回复返回前，`Tool grounding guard` 会让 verifier 模型把答案拆成少量 claim，并标注哪些 claim 需要“本轮工具证据”以及对应的 tool call id。Runtime 只校验这些 id 是否真实存在且成功，conversation history 和 memory 只能作为上下文，不能作为外部事实、当前状态或副作用的证据。若 verifier 发现缺少本轮证据，Agent 会自动重试一次并要求调用合适工具；重试后仍然缺证据时，会优先让 repairer 模型删除或降级 unsupported claim 并再次校验，只有修复后仍然不可信时才替换为保护性说明。建议类回复不会被拦截，例如“你可以运行以下命令”不会被当成已执行结果。
...
- [x] 工具证据保护：最终回复先经过 claim/evidence verifier，只有引用到本轮成功 tool call id 的外部事实、当前状态和副作用声明才被视为已证实；重试后仍有 unsupported claim 时优先修复答案而非整段拦截
