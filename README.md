...
计划确认模块在 channel 层接入：当用户消息包含 `plan` 或“计划”时，系统会先调用轻量模型生成执行计划，把待确认计划保存到 `.ternura/sessions/<session-id>/pending_plan.json`，并要求用户回复 `确认执行`、`取消` 或修改意见。只有确认后，原始任务才会重新进入 schedule shortcut 或普通 Agent 执行链路。
...
```text
用户输入 plan 任务 -> 生成计划并落盘 -> 等待用户确认/取消/修改 -> 确认后执行原任务
```
...
- [x] Plan 确认流：用户消息包含 `plan` 或“计划”时先生成计划并落盘，确认后才执行原任务
