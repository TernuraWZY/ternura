...
- 支持连续对话上下文
- ContextBuilder 按 Learn Claude Code `s08_context_compact` 构建，支持完整消息链、运行时上下文优先级、分层压缩、工具结果持久化和记忆按需注入
- 支持工具调用和工具结果回传
...
Daemon 侧采用轻量 PI-style session 边界，而不是引入重型前置路由框架：外部 channel 和后台 runner 只负责把输入整理成 `agentSessionRunRequest`，其中区分展示给用户的 `DisplayMessage`、实际进入模型的 `RuntimePrompt`、触发来源和是否跳过对话历史写入；`agentSession` 统一负责创建 run、恢复 session 上下文、执行 Agent 或写入直接结果、记录状态和持久化最终输出。这样 Feishu、cron 和后续 channel 可以复用同一套运行生命周期，避免每个入口各自拼 start/run/finish 逻辑。

ContextBuilder 按 Learn Claude Code `s08_context_compact` 的分层压缩顺序构建，并按 Eino 的消息结构做 Go 版映射。它不会在进入模型前单独删除历史 assistant 或历史 tool exchange；历史 user / assistant / tool 链必须成组保留，避免旧 user 指令变成没有回答的“裸消息”。每次模型调用前会按顺序执行：

1. `tool_result_budget`：只统计最新一组连续 tool result；如果总量超过预算，会把超过阈值的大工具输出写入 `.task_outputs/tool-results/<tool_call_id>.txt`，模型上下文只保留文件路径和 preview
2. `snip_compact`：消息数量过多时裁掉中间对话，并在边界上保护 assistant tool call 与后续 tool result 不被拆散
3. `micro_compact`：把较旧工具结果替换为 `[Earlier tool result compacted. Re-run if needed.]` 占位，保留最近几条完整工具结果
4. `compact_history`：如果确定性压缩后仍超过摘要阈值，或模型主动调用 `compact` 工具，会先把完整 transcript 写入 `.transcripts/`，再调用模型生成 Markdown 摘要，并把下一次业务模型调用压成 system + `[Compacted]` summary
5. 最终字符预算裁剪：始终保留最新用户消息，并按 tool-call group 维度从尾部保留上下文，避免留下孤立 tool result

LLM 全量摘要 compact 的位置保持和 Learn Claude Code `s08_context_compact` 一致：它只在 `tool_result_budget -> snip_compact -> micro_compact` 之后触发，不会抢在确定性压缩之前运行。若 Provider 返回 `prompt_too_long`、`context length` 或类似错误，Harness 会执行 reactive compact：写 transcript、生成 `[Compacted]` summary、保留最近尾部消息并重试一次。摘要失败时会记录 `Context compact` trace，并回退到最终预算裁剪，避免单轮运行直接中断。
...
- 如果 Agent 达到最大步骤数或运行异常，session 层会持久化一条可读失败结果，飞书侧会把这条失败说明发回用户，避免只在 daemon 日志里静默失败
...
- [x] PI-style session 边界：把 Feishu、cron 等入口统一收敛为 `agentSessionRunRequest`，由 `agentSession` 管理 DisplayMessage、RuntimePrompt、直接结果、模型执行和持久化
...
- [x] 上下文压缩管线：参考 Learn Claude Code `s08_context_compact`，在模型调用前执行 tool-result budget、snip compact、micro compact、LLM 全量摘要 compact、reactive compact、`compact` 工具触发和 tool-pair-safe 预算裁剪
- [ ] 深度上下文管理：增加 token 预算统计、compact 后跨 run 摘要恢复策略和压缩质量评估
...
- [ ] 外部端事件协议增强：在 `agentSessionRunRequest` 之上继续补充事件序号、状态回放和错误事件规范
