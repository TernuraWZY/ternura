...
- 内置状态归因 Guard：对定时任务、命令执行、安装、外部查询、文件修改和记忆修改这类真实结果做最终回复校验，防止模型把未执行的操作说成事实
...
Harness 还会执行通用工具证据校验：如果最终回复像是在汇报命令输出、安装成功、外部查询结果、文件修改或记忆修改，但本轮 trace / RunContext 中没有对应工具证据，`Tool grounding guard` 会先把对应工具设置为 required 并自动重试一次，让模型必须真实调用工具后再回答；如果重试后仍然没有工具证据，回复才会被替换为保护性说明，并在 trace 中追加 guard 记录。建议类回复不会被拦截，例如“你可以运行以下命令”不会被当成已执行结果。
...
- [x] 工具证据保护：对命令输出、安装成功、外部查询、文件修改和记忆修改做最终回复证据校验，没有对应工具 trace 时替换为保护性说明
