...
- 支持连续对话上下文
- ContextBuilder 支持运行时上下文优先级、字符预算、历史消息裁剪、工具结果压缩和记忆按需注入
- 支持工具调用和工具结果回传
...
ContextBuilder 参考 Learn Claude Code `s08_context_compact` 的分层压缩思路，但按 Eino 的消息结构做了 Go 版映射。每次模型调用前会按顺序执行：

1. `tool_result_budget`：先统计当前上下文里的 tool result 总量，超出预算时优先压缩较旧或过大的工具结果，只保留 preview 和 tool/call id
2. `snip_compact`：消息数量过多时裁掉中间对话，并在边界上保护 assistant tool call 与后续 tool result 不被拆散
3. `micro_compact`：把较旧的当前 run 工具结果替换为占位 preview，保留最近几条完整工具结果
4. `compact_history`：如果确定性压缩后仍超过摘要阈值，先调用模型把较早对话压成 Markdown 摘要，再把摘要和最近消息 / tool result 一起交给真正的业务模型调用
5. 最终字符预算裁剪：始终保留最新用户消息，并按 tool-call group 维度从尾部保留上下文，避免留下孤立 tool result

LLM 全量摘要 compact 的位置保持和 Learn Claude Code `s08_context_compact` 一致：它只在 `tool_result_budget -> snip_compact -> micro_compact` 之后触发，不会抢在确定性压缩之前运行；摘要失败时会记录 `Context compact` trace，并回退到最终预算裁剪，避免单轮运行直接中断。
...
- [x] 上下文压缩管线：参考 Learn Claude Code `s08_context_compact`，在模型调用前执行 tool-result budget、snip compact、micro compact、LLM 全量摘要 compact 和 tool-pair-safe 预算裁剪
- [ ] 深度上下文管理：增加 token 预算统计、reactive compact 和 compact 后上下文恢复策略
