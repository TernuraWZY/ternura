...
- 内置 `read`、`write`、`edit`、`bash`、`update_todos`、`remember`、`forget_memory`、`schedule_task` 和 `cancel_scheduled_task` 工具
...
- 支持一次性定时任务：任务持久化后由后台 runner 到点触发 Agent，并把结果写回对应 session
...
- Schedule 按钮：打开定时任务弹窗，创建一次性定时任务，查看任务状态，并取消尚未开始的任务
- Memory 按钮：打开记忆弹窗，用独立卡片查看长期记忆和当前 session 的短期记忆；长期记忆支持按 memory id 删除
...
`done` 事件会携带最终的 trace 快照，前端会用它对齐流式过程中产生的 trace，确保 `FinalizeRun` 阶段追加的 Guard 记录也能显示在 `Reasoning & tool use` 中。
...
- `schedules.json`：一次性定时任务列表，包括任务标题、prompt、目标 session、触发时间、状态、对应 run id 和错误信息
...
`/api/memory/status` 返回当前长期记忆数量和当前 session 的短期记忆轮次数。`GET /api/memory` 返回长期记忆列表和当前 session 短期记忆详情，`DELETE /api/memory` 按 memory id 删除长期记忆。模型调用前会通过 Hook 注入一个 `Memory` runtime context block，包含长期记忆、当前 session 短期摘要和最近轮次。长期记忆只应该保存稳定、可复用的用户偏好、项目事实或常驻指令；不要保存 API Key、密码或一次性细节。

`GET /api/schedules` 返回所有定时任务。`POST /api/schedules` 创建一次性任务，参数包括 `title`、`prompt`、可选 `run_at` / `delay_seconds` 和可选 `session_id`，其中 `run_at` 使用 RFC3339 时间，`delay_seconds` 用于“2 分钟后”这类相对时间。后端会拒绝明显位于过去的 `run_at`，避免模型用错误占位时间创建任务。`DELETE /api/schedules` 按任务 id 取消尚未开始的任务。Web Console 会轮询定时任务状态；检测到任务完成或失败后会刷新 History，如果任务属于当前 session，会自动把新 run 渲染到聊天区并在 Events 里提示。后台 runner 每秒检查一次到期任务，并按任务绑定的 session 恢复上下文、执行 Agent、写入 run 记录和对话历史。

对于“2 分钟后吃饭提醒”“设置 30 秒后提醒我洗澡”这类简单相对时间提醒，后端会先走确定性解析并直接创建定时任务，不依赖模型自行调用工具；这样可以避免模型只回复“已设置”但实际没有写入 `schedules.json` 的情况。更复杂的时间表达仍由模型通过 `schedule_task` 工具创建。

模型调用前会通过 `current_time` Hook 注入当前时间和时区，通过 `schedule_guidance` Hook 对明显的提醒、定时、闹钟、取消提醒等意图注入工具调用指导：相对时间优先使用 `delay_seconds`，绝对时间必须使用带时区的 RFC3339 `run_at`，最终回复只能报告工具真实返回的任务 id。普通的未来时间问题，例如“明天天气怎么样”，不会触发这段调度指导。

Harness 会在最终回复返回前执行状态归因校验：如果模型声称创建了定时任务、返回了 `schedule-...` id，但本轮没有成功的 `schedule_task` 结果，且该 id 也不存在于真实 schedule store 中，最终回复会被替换为保护性说明，并在 `Reasoning & tool use` 中追加 `Harness guard` trace。前端只应把 `/api/schedules` 和工具结果视为真实任务状态，不能把模型文本里的 id 当成事实来源。
...
├── main/memory_hook.go # 记忆 Hook，负责注入 runtime context 并更新短期记忆
├── main/memory_store.go # 长期/短期记忆的本地持久化
├── main/main.go      # CLI 和 Web 服务入口
├── main/runtime_guidance_hook.go # 当前时间和调度意图指导 Hook
├── main/scheduler.go # 后台定时任务 runner
├── main/schedule_store.go # 一次性定时任务持久化
...
`RunContext` 会记录本轮已经执行过的工具结果，Hook 可以据此判断最终回复是否有真实工具产物支撑。当前 `state_guard` 就利用这个工具结果账本和 `.ternura/schedules.json`，拦截“模型说已设置，但后端没有任务”的情况。
...
- `schedule_task`：模型在用户明确要求稍后提醒、检查、继续或执行任务时创建一次性定时任务
- `cancel_scheduled_task`：模型在用户要求取消任务时按 schedule id 取消尚未开始的任务
- 后台 runner：服务启动后每秒检查 `.ternura/schedules.json`，到点后用任务绑定的 session 恢复对话上下文，执行 Agent，并把结果保存为该 session 的新 run
...
- `schedule_task`：创建一次性定时任务；相对时间使用 `delay_seconds`，绝对时间使用带时区的 RFC3339 `run_at`
- `cancel_scheduled_task`：按 schedule id 取消尚未开始的定时任务
...
- [x] 一次性定时任务：提供 `schedule_task` / `cancel_scheduled_task` 工具、Schedule 弹窗和后台 runner，到点后恢复 session 上下文并执行 Agent
- [x] 定时任务指令遵循：通过当前时间 runtime context、定时意图识别和工具 schema 约束，引导模型真实调用 `schedule_task` / `cancel_scheduled_task`
- [ ] 循环定时任务：支持 interval、daily、weekly 和 cron-like schedule，并记录下一次触发时间
...
```bash
gofmt -w agent.go hook.go main/*.go shared/config.go tool/*.go prompt.go
```
