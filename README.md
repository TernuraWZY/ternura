...
├── main/cron/        # nanobot-style cron store、schedule 计算和运行历史
├── main/feishu/      # Feishu/Lark 事件回调、消息解析和 OpenAPI 发送
├── main/cron_runner.go # 后台定时任务 runner
├── main/feishu_channel.go # 飞书消息到 Agent session 的桥接
...
- `main/feishu`：处理飞书长连接事件、URL verification、事件 token 校验、消息去重、文本/富文本解析、群聊 @ 策略和 OpenAPI 发消息
- `main/feishu_channel.go`：把飞书消息映射为稳定 session id，运行独立 Agent，并为飞书来源的 `cron` job 附加回传地址
