...
Agent 创建时会先装配 `SkillRegistry`。一个 Skill 是一组面向某类能力的说明、工具和 Hook，例如当前内置的 `workspace`、`memory`、`schedule` 和 `web`。Registry 会去重工具、合并 Hook，并在模型调用前注入 `Enabled Skills` runtime context，让模型知道当前启用了哪些能力。
...
```text
用户输入 -> SkillRegistry -> runtime context -> 模型 -> 工具调用 -> 本地工具 -> 工具结果 -> 模型 -> 最终回复
```
