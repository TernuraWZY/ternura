...
模型调用前会通过 `active_memory` Hook 构造 Active Memory recall：先调用轻量模型把当前用户消息和最近几轮对话提取成 keywords/search query，再按 query 相关性召回长期记忆、当前 session 最近轮次和工具结果摘要，最后注入一个 `Active Memory` runtime context block。若关键词提取失败，会回退到本地规则 query，避免记忆模块阻断主 Agent；长期记忆只应该保存稳定、可复用的用户偏好、项目事实或常驻指令，不要保存 API Key、密码或一次性细节。
...
计划确认模块在 channel 层接入：系统会先调用轻量模型判断用户是否想进入 Plan Mode、是否确认执行、取消或修改待确认计划。进入 Plan Mode 后，系统会生成执行计划，把待确认计划保存到 `.ternura/sessions/<session-id>/pending_plan.json`，并要求用户确认、取消或提出修改意见。只有模型路由判断用户已明确确认后，原始任务才会重新进入 schedule shortcut 或普通 Agent 执行链路。
...
```text
用户表达先计划再确认的意图 -> AI 路由进入 Plan Mode -> 生成计划并落盘 -> AI 判断确认/取消/修改 -> 确认后执行原任务
```
...
- [x] Plan 确认流：通过 AI 路由判断是否进入计划、确认、取消或修改，确认后才执行原任务
